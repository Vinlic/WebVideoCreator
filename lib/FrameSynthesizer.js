...
import { SUPPORT_FORMAT, VIDEO_CODEC, AUDIO_CODEC, FORMAT_VIDEO_CODEC_MAP, FORMAT_AUDIO_CODEC_MAP } from "./Preset.js";
...
export default class FrameSynthesizer extends EventEmitter {
    ...
    // 视频编码器映射
    static VCODEC = VIDEO_CODEC;
    // 音频编码器映射
    static ACODEC = AUDIO_CODEC;
     ...
     * @param {string} [options.coverCaptureFormat=png] - 封面捕获格式
    ...
    constructor(options) {
        ...
        const { width, height, fps, duration, format,
            outputPath, attachCoverPath, coverCapture, coverCaptureTime,
            coverCaptureFormat, liveUrl, videoCodec, videoQuality, videoBitrate,
            pixelFormat, audioCodec, audioBitrate, volume, parallelWriteFrames } = options;
        ...
        this.coverCaptureFormat = _.defaultTo(coverCaptureFormat, "png");
        ...
        this.#swapFilePath = path.join(this.#tmpDirPath, `${uniqid("video_")}.${this.format}`);
    ...
    start() {
        ...
        (async () => {
            ...
            if (this.audioSynthesis) {
                ...
                await fs.remove(this.#swapFilePath);
            }
    ...
    async #captureCover() {
        ...
        assert(["jpg", "png"].includes(coverCaptureFormat), "coverCaptureFormat must be jpg or png");
        let captureTime = 0;
        if(_.isFinite(coverCaptureTime))
            captureTime = Math.min(coverCaptureTime, this.#getVideoDuration());
        else
            captureTime = this.#getVideoDuration() * 0.2;
        const coverPath = path.join(path.dirname(outputPath), `${path.basename(outputPath)}.${coverCaptureFormat}`);
        console.log(coverPath, captureTime)
        await util.captureScreenshot(this.outputPath, coverPath, captureTime);
    }
    ...
    _createVideoEncoder() {
        ...
        vencoder
            ...
            .toFormat(format)
            // 指定输出路径
            .addOutput(this.audioSynthesis ? this.#swapFilePath : outputPath);
    ...
    _createAudioEncoder() {
        ...
        aencoder
            .addInput(this.#swapFilePath)
            .videoCodec("copy")
            .setDuration(this.#getVideoDuration() / 1000)
     ...
     */
    #getVideoDuration() {
        return Math.floor(this.#frameCount / this.fps) * 1000;
    }
     ...
     */
    get audioSynthesis() {
        return !this.isVideoChunk() && this.#audios.length > 0;
    }
