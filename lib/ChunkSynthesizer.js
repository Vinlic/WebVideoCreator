...
export default class ChunkSynthesizer extends Synthesizer {
    ...
    input(chunk, transition) {
        ...
        transition && chunk.setTransition(transition);
    ...
    _createVideoEncoder() {
        ...
        if (attachCoverPath) {
            ...
            complexFilter += `[${inputs.length}:v]scale=${videoWidth}:${videoHeight}[cover];${lastOutput || "[0:v]"}[cover]overlay=repeatlast=0${output};`;
