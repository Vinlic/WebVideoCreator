...
export default {
    ...
    async separateVideoAudioTrack(source, dest, options = {}) {
        ...
        const { seekStart, seekEnd, audioCodec = "libmp3lame", audioBitrate = "320k",
     ...
     * @param {number} [options.seekEnd] - 裁剪结束时间点
     */
    async cutAudio(source, dest, options = {}) {
        assert(_.isObject(options), "options must be Object");
     ...
     * @param {string[]} [options.mimesLimit] - 限制MIME类型列表
     */
    async download(url, dest, options = {}) {
        const { onProgress, mimesLimit, ..._options } = options;
