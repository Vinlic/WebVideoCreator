...
export default class CaptureContext {
    ...
    constructor() {
        ...
        window.clearInterval = timerId => {
            for(let i = 0;i < this.intervalCallbacks.length;i++) {
                if(this.intervalCallbacks[i][0] != timerId)
                    continue;
                this.intervalCallbacks.splice(i, 1);
                break;
            }
        }
        // 暂存setTimeout函数
        ...
        window.clearTimeout = timerId => {
            for(let i = 0;i < this.timeoutCallbacks.length;i++) {
                if(this.timeoutCallbacks[i][0] != timerId)
                    continue;
                this.timeoutCallbacks.splice(i, 1);
                break;
            }
        }
        // 暂存Date.now函数
        ...
        // 重写performance.now函数
        performance.now = () => this.startupTime + this.currentTime;
        // const ____CustomEvent = CustomEvent;
        // class CustomEvent extends ____CustomEvent {
        //     constructor(...args) {
        //         super(args);
        //         Object.defineProperty(this, 'timeStamp', { value: this.currentTime });
        //     }
        // }
    }
    ...
    callTimeoutCallbacks() {
        const removeIndexes = [];
        for(let i = 0;i < this.timeoutCallbacks.length;i++) {
            const [timerId, timestamp, timeout, fn] = this.timeoutCallbacks[i];
        ...
        for(let i = 0;i < removeIndexes.length;i++)
            this.timeoutCallbacks.splice(removeIndexes[i], 1);
    }
    ...
    convertToVideoCanvas(e) {
        ...
        const options = {
            ...
            startTime: parseInt(e.getAttribute("start-time") || e.getAttribute("startTime")) || 0,
            endTime: parseInt(e.getAttribute("end-time") || e.getAttribute("endTime")) || Infinity,
            seekStart: parseInt(e.getAttribute("seek-start") || e.getAttribute("seekStart")),
            seekEnd: parseInt(e.getAttribute("seek-end") || e.getAttribute("seekEnd")),
