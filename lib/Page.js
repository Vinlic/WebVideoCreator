...
export default class Page {
    ...
    #pushFrame(data, timestamp) {
        ...
        if(this.#frameStack.length >= 10) {
            ...
            if(this.#lastFrameTimestamp && this.#padCallback) {
                ...
                if(duration > 0)
                    this.#frameIndex += this.#padCallback(duration);
            }
    ...
    #drainFrames() {
        for(let i = 0;i < this.#frameStack.length;i++) {
            const [data, timestamp] = this.#frameStack.shift();
            if(this.#lastFrameTimestamp && this.#padCallback) {
                const duration = Math.floor(timestamp - this.#lastFrameTimestamp);
                if(duration > 0)
                    this.#frameIndex += this.#padCallback(duration);
            }
            this.#frameCallback && this.#frameCallback(data);
            ...
            const ctx = {
                ...
                start: function () {
                    ...
                    (function nextFrame() {
                        ...
                        window.requestAnimationFrame(nextFrame.bind(this));
                    }).bind(this)();
                }
            }
