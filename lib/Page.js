...
export default class Page extends EventEmitter {
    ...
    async goto(url) {
        ...
        await this.target.evaluate(() => {
            ...
            document.body.appendChild(captureHelper);  //加入到body中
            let rotate = 0;
            (function update() {
        ...
        await this.createCaptureElements();
    }

    /**
     * 获取捕获元素并转换为画布
     */
    async createCaptureElements() {
        await this.target.evaluate(() => {
            const captureCtx = window.captureCtx;
            const videos = document.querySelectorAll("capture-video");
            const images = document.querySelectorAll("capture-image");
            const lotties = document.querySelectorAll("capture-lottie");
            return {
                videos: videos.forEach(e => captureCtx.convertToVideoCanvas(e)),
                images: images.forEach(e => captureCtx.convertToDynamicImage(e)),
                lottie: lotties.forEach(e => captureCtx.convertToLottieCanvas(e))
            };
        });
    }
    ...
    async #envInit() {
        ...
        await this.target.evaluateOnNewDocument(`
            ...
            window.DynamicImage=${VideoCanvas};
