...
export default class Page extends EventEmitter {
    ...
    constructor(parent, options) {
        ...
        assert(_.isFinite(width), "Page width must be number");
        assert(_.isFinite(height), "Page height must be number");
        assert(_.isString(userAgent), "Page userAgent must be string");
        assert(_.isFinite(beginFrameTimeout), "Page beginFrameTimeout must be number");
        assert(_.isString(frameFormat), "Page frameFormat must be string");
        assert(_.isFinite(frameQuality), "Page frameQuality must be number");
        assert(_.isBoolean(_firstPage), "Page _firstPage must be boolean");
        this.width = width;
        this.height = height;
        this.userAgent = userAgent;
        this.beginFrameTimeout = beginFrameTimeout;
    ...
    async startScreencast(options = {}) {
        await asyncLock.acquire("startScreencast", async () => {
            let { fps = 60, duration, frameCount } = options;
