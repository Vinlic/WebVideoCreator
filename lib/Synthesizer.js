...
import {
    SUPPORT_FORMAT, VIDEO_CODEC, AUDIO_CODEC, FORMAT_VIDEO_CODEC_MAP,
...
export default class Synthesizer extends EventEmitter {
    ...
    _createVideoEncoder() {
        ...
        if(this.attachCoverPath) {
            vencoder.addInput(this.attachCoverPath);
            vencoder.complexFilter(`[1:v]scale=${width}:${height}[cover];[0:v][cover]overlay=repeatlast=0,scale=w=${width}:h=${height}`);
        }
        else
            vencoder.setSize(`${width}x${height}`);
        vencoder
            // 使用图像管道
