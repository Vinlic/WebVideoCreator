...
export default class Browser {
     ...
     * @param {boolean} options.useGPU - 是否使用GPU加速渲染
     * @param {boolean} options.useAngle - 3D渲染后端是否使用Angle，建议开启
     * @param {boolean} options.disableDevShm - 是否禁用共享内存，当/dev/shm较小时建议开启此选项
     * @param {boolean} options.args - 浏览器flag
     */
    constructor(parent, options) {
        assert(parent instanceof Pool, "Browser parent must be Pool");
        this.parent = parent;
        assert(_.isObject(options), "Browser options must be object");
        const { numPageMax, numPageMin, executablePath, useGPU = false, useAngle = true, disableDevShm = false, args = [], debug, pageOptions = {} } = options;
        ...
        assert(_.isUndefined(executablePath) || _.isBoolean(executablePath), "Browser options.executablePath must be string");
        assert(_.isBoolean(useGPU), "Browser options.useGPU must be boolean");
        assert(_.isBoolean(useAngle), "Browser options.useAngle must be boolean");
        assert(_.isBoolean(disableDevShm), "Browser options.disableDevShm must be boolean");
        assert(_.isArray(args), "Browser options.args must be array");
